...
** 位於ONTAP FAS或AFF主系統和輔助系統上的數據，並使用NetApp備份和恢復整合保護到本地StorageGRID物件存儲


* 內部部署於混合雲中、以保護下列項目：
+
** 資料位於ONTAP FAS 不實或AFF 不實的系統上、並複寫至Cloud Volumes ONTAP 不實
** 位於ONTAP FAS或AFF主系統和輔助系統上的數據，使用NetApp備份和恢復整合保護到雲端中的物件和歸檔存儲
...
+
如需 Windows Cmdlet 的詳細資訊，請參閱 https://docs.netapp.com/us-en/snapcenter-cmdlets-48/index.html["《軟件指令程式參考指南》SnapCenter"^]。
