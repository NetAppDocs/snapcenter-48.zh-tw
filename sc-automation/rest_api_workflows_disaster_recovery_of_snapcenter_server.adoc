...
+
*** 災難恢復還原之後、將會新增主機、但您應該手動安裝外掛程式。
*** 只有當您安裝SnapCenter 適用於Windows的還原外掛程式、並將NetApp LUN附加至伺服器機器時、儲存庫備份排程才會還原。
*** 如果 DLL 毀損、您可以嘗試修復 SnapCenter 伺服器或修正錯誤的安裝。
*** 如果 NSM 或組態檔案毀損、您可以使用相同版本解除安裝並重新安裝 SnapCenter 伺服器。
*** 如果虛擬機器已損壞，請啟動另一台虛擬機器或具有相同名稱的機器並安裝具有相同版本的SnapCenter Server。
